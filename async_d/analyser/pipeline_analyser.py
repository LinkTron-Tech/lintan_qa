...
class PipelineAnalyser(Analyser):
    ...
    def __init__(self):
        ...
        self.func_info = {}
        self.func_lock = defaultdict(lambda: threading.Lock())
    ...
    def decorator(self, func):
        ...
        func_name = f"{orig.__name__}_{hex(id(orig))}"
        func_info = self.func_info[func_name]
        
        lock = self.func_lock[func_name]
        if inspect.iscoroutinefunction(func):
            @functools.wraps(func)
            async def exec_time_wrapper(*args, **kwargs):
            ...
            def exec_time_wrapper(*args, **kwargs):
                # func_name = f"{func.__name__}_{hex(id(func))}"
                # func_info = self.func_info[func_name]
